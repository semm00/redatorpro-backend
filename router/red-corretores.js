...
router.post("/", authMiddleware, upload.single("imagem"), async (req, res) => {
  ...
  if ((!texto || !texto.trim()) && !file) {
    return res
      .status(400)
      .json({ error: "Envie o texto digitado OU a imagem da redação." });
  }
  if (texto && texto.trim() && file) {
    return res
      .status(400)
...
router.get("/solicitacoes", authMiddleware, async (req, res) => {
  try {
    ...
    if (statusFilter === "pendente") {
      whereClause.corrections = { none: { corretorId } };
    } else if (statusFilter === "corrigida") {
      whereClause.corrections = { some: { corretorId } };
    }
    ...
    const mapped = essays.map((e) => {
      const latestCorrection = e.corrections?.[0] || null;
      const notaTotal =
        typeof latestCorrection?.notaTotal === "number"
          ? latestCorrection.notaTotal
          : typeof e.notaTotal === "number"
            ? e.notaTotal
            : null;
      ...
      return {
        ...
        status: latestCorrection ? "Corrigida" : "Pendente",
        aluno: e.author
          ? {
              id: e.author.id,
              name: e.author.name,
          ...
          : null,
        notas: latestCorrection?.notas ?? null,
        notaTotal,
        comentariosGerais: latestCorrection?.comentariosGerais ?? null,
        corrigidaEm: latestCorrection?.updatedAt ?? null,
        correctionId: latestCorrection?.id ?? null,
      };
    });
...
router.get("/pendentes", async (req, res) => {
  ...
  try {
    ...
    const mapped = essays.map((e) => ({
      ...
      imagemUrl: e.urlImage,
      status:
        e.notaTotal !== undefined && e.notaTotal !== null
          ? "Corrigida"
          : "Pendente",
...
router.patch("/:id/visualizada", authMiddleware, async (req, res) => {
  try {
    ...
    const updated = await prisma.essay.update({
      ...
      select: {
        id: true,
        correcaoVisualizada: true,
        correcaoVisualizadaEm: true,
      },
    });
...
router.get("/:id/avaliacao", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json(
      rating ? { value: rating.value, createdAt: rating.createdAt } : null
    );
  } catch (err) {
...
router.post("/:id/avaliar", authMiddleware, async (req, res) => {
  try {
    ...
    if (!essayId || !autorUserId || !Number.isFinite(r) || r < 0.5 || r > 5) {
      return res
        .status(400)
        .json({ error: "Rating inválido. Deve ser entre 0.5 e 5." });
    }
    ...
    if (!essay)
      return res.status(404).json({ error: "Redação não encontrada." });
    if (essay.authorId !== autorUserId)
      return res
        .status(403)
        .json({ error: "Só o autor pode avaliar esta correção." });
    ...
    if (!existingCorrection)
      return res
        .status(400)
        .json({ error: "Correção ainda não disponível para avaliação." });
    ...
    if (existingRating) {
      return res
        .status(200)
        .json({
          message: "Você já avaliou esta correção.",
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const corretorUserId =
        existingCorrection.corretorId || essay.corretorId || null;
    ...
    return res.json({
      ...
      ratingCount: result.updatedCorretor
        ? result.updatedCorretor.ratingCount
        : null,
      ratingSum: result.updatedCorretor
        ? result.updatedCorretor.ratingSum
        : null,
    });
  } catch (err) {
