...
const repertoriosBucket =
  process.env.SUPABASE_REPERTORIOS_BUCKET || "repertorios";
...
function validateRepertorio(body) {
  ...
  if (!body.title || !body.essayUse)
    return "Titulo e uso na redacao sao obrigatorios.";
  return "";
}
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!repertorio)
      return res.status(404).json({ error: "Repertorio nao encontrado." });
    res.json(repertorio);
  } catch (err) {
